...
public class WoodController {
    ...
    @GetMapping("/bundle")
    public ResponseEntity<?> getBundles(@RequestParam String type, // Change WoodType to String
                                        @RequestParam(required = false) Double minPrice,
                                        @RequestParam(required = false) Double maxPrice) {
        List<Bundle> bundles = woodService.getBundles(type, minPrice, maxPrice); // Pass the String type directly
